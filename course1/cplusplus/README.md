...
Реализовать класс ```rational```, который содержит: 
* ```rational(int)``` – Конструктор от целого 
* ```rational(int, int)``` – Конструктор от числителя и знаменателя 
* ```int getNum() const;``` - Получить числитель 
* ```int getDenom() const;``` - Получить знаменатель 
* ```rational operator +(rational const &) const``` - Сложение 
* ```rational operator -(rational const &) const``` - Вычитание 
* ```rational operator *(rational const &) const``` - Умножение 
* ```rational operator /(rational const &) const``` – Деление 

Числитель и знаменатель должны поддерживаться взаимно простыми 

Сборка осуществляется: 
* ```cmake -DCMAKE_INSTALL_PREFIX=<install_dir>``` 
* ```make install``` 

После сборки в install_dir должны быть следующие файлы: 

* include/ 
 - rational.h 
* lib 
 - librational.so либо librational.a 
